...
h19v12<-"F:/MODIS_ET-8day/h19v12"
shape<- readShapePoints("D:/work/Data/Validation_ET/Sites/h19v12_Validation_DataBase_Ver1_Unique_Name.shp")
...
for (n in 1:nfiles)
	...
	xstart <- abs((as.numeric(box[1])-rastx)/0.0086273)
	ystart <- abs((as.numeric(box[4])-(rasty))/0.0086273)
	imgs<-data.frame((list.files(h19v12, pattern=".tif",full.names=FALSE)),(list.files(h19v12, pattern=".tif", full.names=TRUE)))
...
###################################################################################################################
	...
	dd<-paste(substr(as.vector(names(bind)),2,9),".txt",sep="")
	write.table(bind,dd)
	rm(bind,Lst_mean)
