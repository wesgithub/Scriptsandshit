...
h20v12<-"F:/MODIS_ET-8day/h20v12"
shape<- readShapePoints("D:/work/Data/Validation_ET/Sites/h20v12_Validation_DataBase_Ver1_Unique_Name.shp")

rastx<-23.0940108
rasty<--30.0000000
res<-0.008627301502585
...
for (n in 1:nfiles)
	...
	imgs<-data.frame((list.files(h20v12, pattern=".tif",full.names=FALSE)),(list.files(h20v12, pattern=".tif", full.names=TRUE)))
...
###################################################################################################################
	...
	dd<-paste("h20v12_",substr(as.vector(names(bind)),2,8),".txt",sep="")
	write.table(bind,dd)
	rm(bind,Lst_mean)
