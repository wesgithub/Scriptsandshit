...
c<-a[,2:b]
#c[c==0]<-NA
d<-zooreg(c, start = as.Date("1981-11-18"))
e<-window(d, start = as.Date("1981-11-18"), end = as.Date("2011-04-11"))
names(e) <-c("Flow","Qual")
...
rastx<-16.00
rasty<--21.80
vector <- readShapePoly("D:/work/Data/Validation_CCAM/Data/scratch/flow/Validation_Analysis/Site_4/A22/A22.shp")
...
m_dish.zoo.win<-window(m_disch.zoo, start = as.Date("1989-01-01"), end = as.Date("2009-12-31"))
e.zoo<-window(e[,1], start = as.Date("1989-01-01"), end = as.Date("2009-12-31"))
...
pdf("A22_Annual_Monthly_Daily.pdf")
...
pdf("A22_Time_Diff_Daily.pdf")
Daily_Diff<-Daily.agg[,1]-Daily.agg[,2]
Daily.agg <- cbind(Daily.agg, Daily_Diff)
plot(Daily.agg, main="A22 Daily Time Series", xlab="Time")
dev.off()

#############Cumulative sum plots#############

pdf("A22_CumSum_Daily.pdf")
plot(cumsum(Daily.agg[,1]),col=2, ylab="Cumulative Sum",main="A22 Cumulative Sum: Run-off")
...
Daily.sea <- window(Daily.agg, start = as.Date("1989-01-01"), end = as.Date("2009-12-31"))
