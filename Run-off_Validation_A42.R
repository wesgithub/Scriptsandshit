...
c<-a[,2:b]
#c[c==0]<-NA
d<-zooreg(c, start = as.Date("2004-02-24"))
e<-window(d, start = as.Date("2004-02-24"), end = as.Date("2011-02-23"))
w=((e[,1]*1000)*86400)/as.numeric(paste(Area))
e<-cbind(e,w)
names(e) <-c("Flow","Qual","AreaAve")
...
rastx<-16.00
rasty<--21.80
vector <- readShapePoly("D:/work/Data/Validation_CCAM/Data/scratch/flow/Validation_Analysis/Site_2/A42/A42.shp")
...
nfiles <- length(maps$Path) 
	...
	out5<-(out5*1000000)/8387.19757188
...
pdf("A42_Annual_Monthly_Daily.pdf")
...
pdf("A42_Time_Diff_Daily.pdf")
Daily_Diff<-Daily.agg[,1]-Daily.agg[,2]
Daily.agg <- cbind(Daily.agg, Daily_Diff)
plot(Daily.agg, main="A42 Daily Time Series", xlab="Time")
dev.off()

#############Cumulative sum plots#############

pdf("A42_CumSum_Daily.pdf")
plot(cumsum(Daily.agg[,1]),col=2, ylab="Cumulative Sum",main="A42 Cumulative Sum: Run-off")
